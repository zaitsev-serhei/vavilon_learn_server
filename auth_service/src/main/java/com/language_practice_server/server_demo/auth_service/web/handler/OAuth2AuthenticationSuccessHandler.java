...
import com.language_practice_server.server_demo.auth_service.controller.OAuth2InitiateController;
...
public class OAuth2AuthenticationSuccessHandler implements AuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException {
        ...
        if (user == null) {
            UserDto u = new UserDto(null, finalFirstName, finalLastName, email);
            u.setRole("TEACHER"); // default role
            logger.debug("User was not found in the DB. Creating new user {}", u);
            userService.createUser(u);


            logger.info("Creating new refresh token for userId:");
            String serverRefreshToken = refreshTokenService.createRefreshTokenForUser(user.getUserId(),
                    providerRefreshToken, registrationId);
            logger.info("Creating new access token for userId: {}", user.getUserId());
            ...
            logger.info("Setting secured cookies for user: {}", user.getUserId());
        }
